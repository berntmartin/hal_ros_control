...
using ProbeCaptureType =
    decltype(stop_event_msgs::SetNextProbeMoveRequest::PROBE_NONE);

/**
 * Communicates status of a probe in realtime from the control loop to any
...
public:
  ...
  ProbeHandle(const std::string& name,  //!< Name of the probe
              int* probe_capture_ptr,   //!< Capture setting (i.e. rising means
                                        //!< expect a rising edge, none implies
              //!< we expect the probe signal to be off
              //!< and no edges to be seen)
              const int* probe_state_ptr,  //!< Probe state signal
              int* probe_transition_ptr,  //!< transitions detected in the probe
                                          //!< signal (handshake)
              int* probe_result_type_ptr,  //!< Kind of transition captured for
                                           //!< the result
  ...
  {
    return probe_result_type_ ?
               static_cast<ProbeTransitions>(*probe_result_type_) :
               ProbeTransitions::INVALID;
  }
    ...
    *probe_capture_ = capture_type;
    ...
    switch (capture_type)
    {
      ...
      case 5:
        ...
        // If we're NOT expecting a capture, then keep the previous result
        // around for querying
  ...
  static ProbeTransitions
  transitionNeededForCapture(ProbeCaptureType capture_type)
  {
    switch (capture_type)
    {
...
/** \brief Hardware interface to support reading the state of a force-torque
 * sensor. */
class ProbeInterface : public hardware_interface::HardwareResourceManager<
                           ProbeHandle, hardware_interface::ClaimResources>
{
};
