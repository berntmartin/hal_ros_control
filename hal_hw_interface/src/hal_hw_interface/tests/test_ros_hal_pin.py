...
data_cases = [
    dict(  # 0 Test random
        last_value=(True, 27, 49, 3.98),
        pin_value=(False, 23, -4, 1e-9),
        msg_data=(True, 27, 49, 3.98),
    ),
    dict(  # 1 Test random
        last_value=(False, 199, -33399, 149.33285),
        pin_value=(True, 0, -39958, 54.7),
        msg_data=(False, 928885, -39957, -1.399e8),
    ),
    dict(  # 2 Test zeros -> random
        last_value=(False, 0, 0, 0.0),
        pin_value=(False, 0, 0, 0.0),
        msg_data=(True, 928885, -39957, -1.399e8),
        changed=False,
    ),
    dict(  # 3 Test data unchanged
        last_value=(True, 199, -33399, 149.33285),
        pin_value=(True, 199, -33399, 149.33285),
        msg_data=(True, 199, -33399, 149.33285),
        changed=False,
    ),
]
...
class TestRosHalPin(object):
    ...
    def msg_data(self, obj, param):
        index = self.data_indexes.get(obj.hal_type)
        return param['msg_data'][index]
    ...
    def test_ros_hal_pin_data_fixture(self, data):
        for key in ('last_value', 'pin_value', 'msg_data'):
            assert key in data
...
class TestRosHalPinSubscriber(TestRosHalPinPublisher):
    ...
    def test_ros_hal_pin_subscriber_cb(self, obj, data, mock_comp_obj):
        ...
        '''
        msg_data = self.msg_data(obj, data)
        obj._subscriber_cb(self.msg_type(obj)(msg_data))
        mock_comp_obj.__setitem__.assert_called_with(
            self.obj_test_name(obj), msg_data
        )
